...
Use this skill to drive a PRD-to-browser-execution QA workflow. The workflow is intentionally gated: stop after every phase and wait for explicit user confirmation before continuing.
...
- Generate executable Browser Bridge steps only after semantic test cases are approved.
- Execute browser actions only after the user confirms the executable plan and browser authorization scope.
- Final output must be an HTML report containing case results, screenshots, and console summaries/details.
- If console capture is unavailable or incomplete, mark it clearly in the report and list it as a development gap.
...
10. `/browser-bridge_qa_run`
   Run approved cases through Browser Bridge. Capture screenshots and console evidence. Use `captureConsole: true`, `failOnConsoleError: true`, and `failOnUncaughtException: true` unless the user explicitly lowers strictness. Stop with a concise run summary.
...
For executable cases, output Browser Bridge-ready JSON or structured steps. Include selectors only when needed and prefer robust label/text/role-based targets.
...
- Prefer deterministic Browser Bridge tools for opening pages, selecting controls, clicking, filling, screenshotting, console monitoring, and QA report generation.
...
- Capture at least one screenshot per case. Capture extra screenshots around high-risk assertions when available.
- Observe console errors/exceptions as pass/fail evidence. Warnings should be shown in the report but do not fail by default unless the case requires it.
