...
```sh
pnpm --filter @browser-bridge/mcp-server smoke -- --tool browser_screenshot --args '{"mode":"cdp","scale":2,"format":"png"}'
```
...
`browser_screenshot` 会返回 MCP image content，适合需要让支持图片内容的客户端直接查看截图；截图不清晰时可传 `{"mode":"cdp","scale":2}`。`browser_save_screenshot` 会把图片写入本地文件，只返回路径和元数据，适合避免大图 base64 占用模型上下文。`browser_run_steps` 中的 `screenshot` 步骤默认只返回截图元数据，不返回完整图片。
